        ...
        <div ref="display" class="message-list">
            <div  class="message-list-content min_h_100">
                <div v-for="(m,index) in msgList" :key="index">
                    <chat-line-right v-if="m.fromId===$store.state.CzpUser.id" :content="m.content" :avatar-url="m.avatar" :type="m.type" :time="m.sendTime"/>
                    <chat-line v-else :content="m.content" :avatar-url="m.avatar" :type="m.type" :time="m.sendTime"/>
                </div>
            </div>
        </div>
        <div class="send-box">
