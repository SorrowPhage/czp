    ...
    <table>
        <tr>
            <td :class="{parentLevel: treeData.children, extend: treeData.children && treeData.extend}"
                ...
                <div :class="{node: true}">
                    <div v-if="treeData.name" class="person" :class="{bacg_home:treeData.id===$store.state.CzpUser.id}" @click="$emit('click-node', treeData)">
        ...
        <tr v-if="treeData.children && treeData.extend">
            <td v-for="(children, index) in treeData.children" :key="index" class="childLevel" colspan="2">
                <HomeTreeChat :json="children" @click-node="$emit('click-node', $event)"/>
            </td>
        </tr>
    </table>
</template>
