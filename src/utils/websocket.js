...
let wsUrl = ''
let tryTime = 0
export let prxHost="ws://localhost:4090/msg/chat/"
...
function initWebSocket () {
    ...
    websock = new WebSocket(wsUrl, [getToken()]);
...
/**
 * 发起websocket请求函数
 * @param {string} userId 连接用户id
 * @param {Object} agentData 传给后台的参数
 * @param {function} successCallback 接收到ws数据，对数据进行处理的回调函数
 * @param {function} errCallback ws连接错误的回调函数
 */
export function sendWebsocket (userId, agentData, successCallback, errCallback) {
    wsUrl = prxHost + userId;
    initWebSocket()
